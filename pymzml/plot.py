...
class Factory(object):
    ...
    def __return_neg_offset_0(self, i):
        """
        """
        return .0 - (self.y_max[i] * (self.offset * 0.05))
    ...
    def add(
        ...
        if len(style) == 3:
            ...
            if pos not in available_pos:
                raise Exception(
                    'Position must one of the following: {0}'.format(
                        available_pos
                    )
                )
        else:
        ...
        if style[0] == 'label':
            ...
            if style[1]  == 'hoverinfo':
                ...
                for x in data:
                    ...
                    y_max = x[1]
                    y_values += .0, y_max, .0, None
                    txt += None, x[2], None, None

                    if pos == 'MS_precision':
                        x_values += x[0] - (x[0] * MS_precision), x[0], x[0] + \
                        (x[0] * MS_precision), None
                        continue
                    elif pos =='micro':
                    ...
                    x_values += x[0] - \
                        (x_max * rel_width), x[0], x[0] + (x_max * rel_width), None
            ...
            x_vals = [mz for mz, i in data if mz_range[0] <= mz <= mz_range[1] and int_range[0] <= i <= int_range[1]]
            y_vals = [i for mz, i in data if mz_range[0] <= mz <= mz_range[1] and int_range[0] <= i <= int_range[1]]
            y_max = max(y_vals)
            y_min = min(y_vals)
            x_max = max(x_vals)
            x_min = min(x_vals)
